...
The `tidy_categorical()` function adds further columns (`variable`, `level` and `effect`) to tidy output to help manage categorical variables
...
Include additional rows for reference category terms and column to indicate their location by setting `exclude_reference = FALSE` and `reference_label` to a suitable string. Setting `exponentiate = TRUE` ensures the parameter estimates in the reference group are set to one instead of zero (even odds in the logistic regression example below).
